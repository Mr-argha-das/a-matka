...
const Sidebar = ({ open }) => {
  ...
  return (
    <div
      className={`fixed lg:z-30 md:z-30 z-40 pt-16 left-0 top-0 h-full bg-[#0b244c]/95 lg:border-r md:border-r border-blue-300/20 text-white shadow-[12px_0_30px_rgba(2,8,23,0.22)] backdrop-blur-xl transition-all duration-300 ${
        open ? "w-64" : "w-0 overflow-hidden"
      }`}
    >
      <div className="flex flex-col h-full p-4 space-y-2">
        {menuItems.map((item, idx) => (
          <div key={idx}>
            {/* Main menu item */}
            <div
              className="flex justify-between items-center text-sm hover:bg-blue-500/20 p-2 rounded-md cursor-pointer"
              onClick={() => {
                if (item.onClick) {
                  item.onClick();
              ...
              <div className="ml-6 mt-1 space-y-1">
                {item.children.map((child, cIdx) => (
                  <a
                    key={cIdx}
                    href={child.link}
                    className="block text-sm text-blue-100 hover:text-white hover:bg-blue-500/20 p-2 rounded-md"
                  >
                    {child.name}
                  </a>
                ))}
