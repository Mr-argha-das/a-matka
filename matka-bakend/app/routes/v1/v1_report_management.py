...
def delete_bid(bid_id: str):
    ...
    return refund_and_delete_bid(bid, "Bid Delete Refund")


def refund_and_delete_bid(bid: Bid, payment_method: str):
    """Refund a bid before removing it so admin actions never lose user points."""
    wallet = Wallet.objects(user_id=bid.user_id).first()
    if not wallet:
        wallet = Wallet(user_id=bid.user_id, balance=0).save()

    wallet.update(inc__balance=bid.points, set__updated_at=datetime.utcnow())
    wallet.reload()
    ...
    Transaction(
        ...
        status="SUCCESS",
        raw_response=f"Refunded and removed bid {bid.id}",
    ).save()

    refunded_points = bid.points
    bid.delete()
    return {
        "message": "Bid removed and points refunded successfully",
...
def revert_bid(bid_id: str):
    ...
    return refund_and_delete_bid(bid, "Bid Revert Refund")
